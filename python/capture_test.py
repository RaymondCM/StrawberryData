...
def capture(win):
	...
	#	print(depth_sensor.get_option(rs.option.exposure))
		...
		sensor_roi = roi_sensor.get_region_of_interest()
		sensor_roi.min_x, sensor_roi.max_x = 0,int(infrared.shape[1]-1)# int(index_min[1]),int( index_min[1] + 1) if index_min[1] +1 < infrared.shape[1] else int(infrared.shape[1]-1)
		sensor_roi.min_y, sensor_roi.max_y = int(infrared.shape[0]/2),int(infrared.shape[0]-1)#int(index_min[0]), int( index_min[0] + 1) if index_min[0] +1 <infrared.shape[0] else int(infrared.shape[0]-1)
		roi_sensor.set_region_of_interest(sensor_roi)
		sensor_roi = roi_sensor.get_region_of_interest()
